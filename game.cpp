	...
	uint16_t spriteId, int16_t fromStackpos, const Position& toPos, uint8_t count)
{
	...
	{
        // <onepiece>
		uint32_t delay = player->hasFlag(PlayerFlag_CanAnswerRuleViolations) ? 0
	...
	const Position& movingCreaturePos, const Position& toPos)
{
	...
	if(!player || player->isRemoved() || player->hasFlag(PlayerFlag_CannotMoveCreatures))
		return false;
	// <onepiece>
	if(!player->canDoMoveAction())
	{
		uint32_t delay = player->getNextMoveActionTime();
	...
	player->setNextMoveAction(OTSYS_TIME() + g_config.getNumber(ConfigManager::MOVE_THING_DELAY));
	...
	uint16_t spriteId, int16_t fromStackpos, const Position& toPos, uint8_t count)
{
    ...
    if(fromPos.x == 0xFFFF)
    {
      ...
      {
        fromIndex = static_cast<uint8_t>(fromPos.z); 
        if(fromCylinder && fromCylinder->getItem())
        {
...
bool Game::playerMove(uint32_t playerId, Direction dir)
{
	...
	if(delay > 0)
	{
		if(g_config.getBool(ConfigManager::POTION_EXAUSTION)) {
			player->setNextAction(OTSYS_TIME() + player->getStepDuration(dir, true) - SCHEDULER_MINTICKS);
		}
...
{
    // <onepiece>
      // Evitar que criaturas causem dano umas �s outras
      if (attacker && ! attacker->getPlayer() && ! attacker->getPlayerMaster()
      &&  target   && ! target->getPlayer()   && ! target->getPlayerMaster())
      {
          //std::cout << "[SUCESS] Creature blocked damage from another creature..." << std::endl;
          return false;
      }      
    // </onepiece>
		...
		if(damage != 0)
		{
			...
			if(damage > 0)
			{
				...
				switch(combatType)
				{
					...
					case COMBAT_DEATHDAMAGE:
					{
						textColor = TEXTCOLOR_ROXOTRES;
						magicEffect = MAGIC_EFFECT_CHUTEDOIS;
						break;
					}
	...
	std::string comment, std::string statement, uint32_t statementId, bool ipBanishment)
{
	...
	{
		player->sendCancel("You do not have authorization for this action on this staff member.");
		return false;
	}
	...
	if(g_config.getBool(ConfigManager::BROADCAST_BANISHMENTS)) {
		ss << player->getName() << " has taken the";
    } else {
		ss      << "You received an ";
		ss_from << "You have taken the ";
    }

	ss      << "action \"" << getAction(action, ipBanishment) << "\"";
	ss_from << "action \"" << getAction(action, ipBanishment) << "\"";
	switch(action)
	{
		case ACTION_NOTATION:
		{
			...
			ss_from << " (" << (g_config.getNumber(ConfigManager::NOTATIONS_TO_BAN) - IOBan::getInstance()->getNotationsCount(account.number)) << " left to banishment)";
			break;
		}
		case ACTION_STATEMENT:
		{
			ss      << " for the statement: \"" << statement << "\"";
			ss_from << " for the statement: \"" << statement << "\"";
			break;
		}
		default:
			break;
    ...
    ss      << " (Warnings: " << account.warnings << "), with reason: \"" << getReason(reason) << "\", and comment: \"" << comment << "\".";
	ss_from << " against: " << name << " (Warnings: " << account.warnings << "), with reason: \"" << getReason(reason) << "\", and comment: \"" << comment << "\".";
		
	if(g_config.getBool(ConfigManager::BROADCAST_BANISHMENTS)) {
		broadcastMessage(ss_from.str(), MSG_STATUS_WARNING);
	} else {
		player->sendTextMessage(MSG_STATUS_CONSOLE_RED, ss_from.str());
		target->sendTextMessage(MSG_STATUS_WARNING, ss.str());
    }
...
{
	int32_t subType = -1;
    /*  -- one piece arrumar look
	if(item->hasSubType() && !item->hasCharges())
		subType = item->getSubType();
    */ 
	const ItemType& it = Item::items[item->getID()];
	uint32_t count = player->__getItemTypeCount(item->getID(), subType);
