...
// <onepiece>
int32_t Player::getSaveManaPercent() const
{
	int32_t mana_percent = 0;
	for(int32_t i = SLOT_FIRST; i < SLOT_LAST; ++i)
	{
		if(Item* item = getInventoryItem((slots_t)i))
			mana_percent += item->getSaveManaPercent();
	}
	return mana_percent;
}

int32_t Player::getExtraLootPercent() const
{
	int32_t extra_loot_percent = 0;
	for(int32_t i = SLOT_FIRST; i < SLOT_LAST; ++i)
	{
		if(Item* item = getInventoryItem((slots_t)i))
			extra_loot_percent += item->getExtraLootPercent();
	}
	return extra_loot_percent;
}

int32_t Player::getVampirismPhysical() const
{
	int32_t vamp = 0;
	for(int32_t i = SLOT_FIRST; i < SLOT_LAST; ++i)
	{
		if(Item* item = getInventoryItem((slots_t)i))
			vamp += item->getVampirismPhysical();
	}
	return vamp;
}

int32_t Player::getVampirismMagic() const
{
	int32_t vamp = 0;
	for(int32_t i = SLOT_FIRST; i < SLOT_LAST; ++i)
	{
		if(Item* item = getInventoryItem((slots_t)i))
			vamp += item->getVampirismMagic();
	}
	return vamp;
}

int32_t Player::getExtraCritical() const
{
	int32_t extra_critical = 0;
	for(int32_t i = SLOT_FIRST; i < SLOT_LAST; ++i)
	{
		if(Item* item = getInventoryItem((slots_t)i))
			extra_critical += item->getExtraCritical();
	}
	return extra_critical;
}

int32_t Player::getHealthRelativeDamage() const
{
	int32_t damage_percent = 0;
	for(int32_t i = SLOT_FIRST; i < SLOT_LAST; ++i)
	{
		if(Item* item = getInventoryItem((slots_t)i))
			damage_percent += item->getHealthRelativeDamage();
	}
	return damage_percent;
}

int32_t Player::getCharmExtraDmgChance() const
{
	int32_t charm_extra_dmg_chance = 0;
	for(int32_t i = SLOT_FIRST; i < SLOT_LAST; ++i)
	{
		if(Item* item = getInventoryItem((slots_t)i))
			charm_extra_dmg_chance += item->getCharmExtraDmgChance();
	}
	return charm_extra_dmg_chance;
}
// </onepiece>
...
int32_t Player::getDefense() const
{
	...
	if(weapon)
	{
		...
		defenseSkill = getWeaponSkill(weapon) + getSkill(SKILL_SHIELD, SKILL_LEVEL);
	}
...
uint32_t Player::getExtraCriticalChance() const
{
            uint32_t bonus = 0;
            // Usopp ganha X% de critical
            if (getBaseVocationId() == 5)
                bonus += g_config.getNumber(ConfigManager::WOP_CRITICAL_USOPP_BONUS);
            if (currentOutfit.lookType == 323 || currentOutfit.lookType == 473)
                bonus += g_config.getNumber(ConfigManager::WOP_CRITICAL_CHOPPER_BRAIN_POINT_BONUS);
            return extraCriticalChance
                 + bonus
                 + getExtraCritical();   // Pegar o bonus de todos os items com a propriedade `extra_critical`
}

// Multiplica o dano dado pelo resultado
float Player::getOutfitExtraAttack() const
{
    //std::cout << "[Debuggin' - Player::getOutfitExtraAttack()] (" << currentOutfit.lookType << "," << ConfigManager::OPC_CHOPPER_OUTFIT_WALK_POINT << ")" << std::endl;
    
    if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_CHOPPER_OUTFIT_WALK_POINT))
        return g_config.getDouble(ConfigManager::OPC_CHOPPER_DAMAGE_WALK_POINT);
    else if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_CHOPPER_OUTFIT_HEAVY_POINT))
        return g_config.getDouble(ConfigManager::OPC_CHOPPER_DAMAGE_HEAVY_POINT);
    else if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_CHOPPER_OUTFIT_GUARD_POINT))
        return g_config.getDouble(ConfigManager::OPC_CHOPPER_DAMAGE_GUARD_POINT);
    else if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_CHOPPER_OUTFIT_GUARD_POINT_ROLLOUT))
        return g_config.getDouble(ConfigManager::OPC_CHOPPER_DAMAGE_GUARD_POINT_ROLLOUT);
    else if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_CHOPPER_OUTFIT_JUMP_POINT))
        return g_config.getDouble(ConfigManager::OPC_CHOPPER_DAMAGE_JUMP_POINT);
    else if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_CHOPPER_OUTFIT_HORN_POINT))         
        return g_config.getDouble(ConfigManager::OPC_CHOPPER_DAMAGE_HORN_POINT);
    else if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_CHOPPER_OUTFIT_ARM_POINT))
        return g_config.getDouble(ConfigManager::OPC_CHOPPER_DAMAGE_ARM_POINT);
    else
        return 1.0f;
}

// Divide o dano tomado pelo resultado
float Player::getOutfitExtraDefense() const
{          
    // Spell `fuusen` deixa o luffy gordo, e protege 70% dos danos 372
    if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_LUFFY_OUTFIT_FUUSEN))
        return g_config.getDouble(ConfigManager::OPC_LUFFY_DEF_OUTFIT_FUUSEN);
    // Spell `gear second` + `fuusen` deixa o luffy gordo e rosa, e protege 75% dos danos 241        
    else if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_LUFFY_OUTFIT_FUUSEN_PINK))
        return g_config.getDouble(ConfigManager::OPC_LUFFY_DEF_OUTFIT_FUUSEN_PINK);
    // [2] Spell `fuusen` deixa o luffy gordo, e protege 70% dos danos 372
    if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_LUFFY_OUTFIT_FUUSEN_2))
        return g_config.getDouble(ConfigManager::OPC_LUFFY_DEF_OUTFIT_FUUSEN_2);
    // [2] Spell `gear second` + `fuusen` deixa o luffy gordo e rosa, e protege 75% dos danos 241        
    else if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_LUFFY_OUTFIT_FUUSEN_PINK_2))
        return g_config.getDouble(ConfigManager::OPC_LUFFY_DEF_OUTFIT_FUUSEN_PINK_2);
    else if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_CHOPPER_OUTFIT_WALK_POINT))
        return g_config.getDouble(ConfigManager::OPC_CHOPPER_DEF_WALK_POINT);
    else if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_CHOPPER_OUTFIT_HEAVY_POINT))
        return g_config.getDouble(ConfigManager::OPC_CHOPPER_DEF_HEAVY_POINT);
    else if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_CHOPPER_OUTFIT_GUARD_POINT))
        return g_config.getDouble(ConfigManager::OPC_CHOPPER_DEF_GUARD_POINT);
    else if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_CHOPPER_OUTFIT_GUARD_POINT_ROLLOUT))
        return g_config.getDouble(ConfigManager::OPC_CHOPPER_DEF_GUARD_POINT_ROLLOUT);
    else if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_CHOPPER_OUTFIT_JUMP_POINT))
        return g_config.getDouble(ConfigManager::OPC_CHOPPER_DEF_JUMP_POINT);
    else if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_CHOPPER_OUTFIT_HORN_POINT))         
        return g_config.getDouble(ConfigManager::OPC_CHOPPER_DEF_HORN_POINT);
    else if(currentOutfit.lookType == g_config.getNumber(ConfigManager::OPC_CHOPPER_OUTFIT_ARM_POINT))
        return g_config.getDouble(ConfigManager::OPC_CHOPPER_DEF_ARM_POINT);
    else
        return 1.0;    
}
// </onepiece>
...
void Player::sendIcons() const
{
	...
	if(!condition || condition->getTicks() <= 5000) {
		icons |= ICON_HUNGRY;
    }
    //if (condition){
    //    std::cout << "1533 __2 food :: (" << condition->getTicks() << ")" << std::endl;
    //}
    // </onepiece>
...
void Player::addSkillAdvance(skills_t skill, uint32_t count, bool useMultiplier/* = true*/)
{
	...
	while(skills[skill][SKILL_TRIES] + count >= nextReqTries)
	{
		...
		s << ".";
		// <onepiece>
        if(skill != SKILL_FISH && skill != SKILL_FIST && skill != SKILL_CLUB)
        {
...
{
        switch(getBaseVocationId()) {
            case 1: return 284;   // sanji
            case 2: return 282;   // luffy
            case 3: return 283;   // zoro
            case 4: return 297;   // ace
            case 5: return 291;   // usopp
            case 6: return 296;   // nami 
            case 7: return 19910; // usopp
            case 8: return 20357; // smoker
            default: // caso n�o for nenhum deles, morre com corpse normal
...
bool Player::canWalkthrough(const Creature* creature) const
{
	...
	if((g_game.getWorldType() == WORLD_TYPE_NO_PVP) && player->getTile() && player->getTile()->ground
		&& (player->getTile()->ground->getID() != ITEM_GLOWING_SWITCH))
		return true;
		
	if(((player->getTile()->hasFlag(TILESTATE_PROTECTIONZONE) || (player->getVocation()->isAttackable()
			&& player->getLevel() < (uint32_t)g_config.getNumber(ConfigManager::PROTECTION_LEVEL))) && player->getTile() && player->getTile()->ground && 
			player->getTile()->ground->getID() != ITEM_GLOWING_SWITCH) && (!player->hasCustomFlag(PlayerCustomFlag_GamemasterPrivileges)
...
void Player::onCreatureAppear(const Creature* creature)
{
	...
	//if player doens't have the soul condition, add it
	if(Condition* findCondition = getCondition(CONDITION_SOUL, CONDITIONID_DEFAULT)) {
		
		if (findCondition->getTicks() > -1) {
			removeCondition(findCondition);
		}
	}
	
	//soul regen is infinite
	if(Condition* condition = Condition::createCondition(
		CONDITIONID_DEFAULT, CONDITION_SOUL, -1))
	{
...
uint32_t Player::getNextMoveActionTime() const
{
	int64_t time = nextMoveAction - OTSYS_TIME();
	if(time < SCHEDULER_MINTICKS)
		return SCHEDULER_MINTICKS;
...
void Player::addManaSpent(uint64_t amount, bool useMultiplier/* = true*/)
{
	...
	while(manaSpent + amount >= nextReqMana)
	{
		...
		char advMsg[50];
		sprintf(advMsg, "You advanced to energy level %d.", magLevel);
...
bool Player::onDeath()
{
	...
	// 	setLossSkill(false);
	// }
	if(skull < SKULL_RED && g_game.getWorldType() != WORLD_TYPE_PVP_ENFORCED)
	{
		Item* item = NULL;
	...
	if(skillLoss)
	{
		...
		if(deathList.size() > 0) {
			if(Creature* firstCreature = deathList[0].getKillerCreature()) {
				if(Player* firstPlayer = firstCreature->getPlayer()) {
					uint32_t sumLevels = 0;
					for(DeathList::iterator it = deathList.begin(); it != deathList.end(); ++it) {
						if(Creature* tmpCreature = it->getKillerCreature()) {
							if(Player* tmpPlayer = tmpCreature->getPlayer()) {
								sumLevels += tmpPlayer->getLevel();
							}
						}
					}	
					
					if(sumLevels > getLevel()) {
						double reduce = getLevel() / static_cast<double>(sumLevels);
		...
		uint64_t lossExperience = getLostExperience() * (unfairFightReduction / 100.);
		removeExperience(lossExperience, false);
		double percent = (1. - ((double)(experience - lossExperience) / experience))  * (unfairFightReduction / 100.);
...
{
    //std::cout << "G4 __WTF creating corpse <Player::>" << std::endl;  
...
void Player::onAddCombatCondition(ConditionType_t type, bool hadCondition)
{
	...
	switch(type)
	{
		...
		case CONDITION_ATTRIBUTES:
			tmp = "strengthened";
			break;
		case CONDITION_FAST_ATTACK:
			tmp = "under fast attack effect";
			break;
		default:
			break;
	}
...
bool Player::gainExperience(double& gainExp, bool fromMonster)
{
	...
	//soul regeneration
	/*
	//removido por cristofer
	//agora a condition de soul � infinita
	if(gainExp >= level)
	{
		if(Condition* condition = Condition::createCondition(
...
void Player::onGainExperience(double& gainExp, bool fromMonster, bool multiplied)
{
    ...
    // <onepiece>
    if (skull >= SKULL_RED) {
        gainExp *= g_config.getDouble(ConfigManager::OPC_RED_SKULL_EXP_PERCENT_LOSS);
    }
    // </onepiece>
...
uint32_t Player::getAttackSpeed()
{
	...
	uint32_t weapon_attack_speed = 0;
	if(weapon && weapon->getAttackSpeed() != 0)
		weapon_attack_speed = weapon->getAttackSpeed();

	return vocation->getAttackSpeed() - getPlayer()->getExtraAttackSpeed() - conditionFastAttack - weapon_attack_speed;
}
