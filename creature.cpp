...
{
    // <onepiece>  -- so alterei o if mesmo pra player com noMove nao poder sair andando ao dar follow rsrsrsrs
	//if(getWalkDelay() <= 0)
	if(getWalkDelay() <= 0 && !cannotMove)
	{
		Direction dir;
...
void Creature::dropCorpse(DeathList deathList)
{
    ...
    //std::cout << "G4 __13" << std::endl;  
	corpse->setParent(NULL);
	if(deny)
		return;
		
    //std::cout << "G4 __14" << std::endl;      
	Tile* tile = getTile();
	if(!tile)
		return;

    //std::cout << "G4 __15" << std::endl;  
	Item* splash = NULL;
	switch(getRace())
	{
		case RACE_VENOM:
...
{
    //std::cout << "G4 __tentando criar corpse item <Creature::>" << std::endl;  
...
int32_t Creature::getStepDuration(Direction dir, bool fixed) const
{
	if(!fixed && (dir == NORTHWEST || dir == NORTHEAST || dir == SOUTHWEST || dir == SOUTHEAST))
		return getStepDuration() * 2;

	return getStepDuration(fixed);
}

int32_t Creature::getStepDuration(bool fixed) const
{
	if(removed)
		return 0;
	...
	int32_t groundSpeed = Item::items[tile->ground->getID()].speed;
	
	if (fixed) {
		groundSpeed = g_config.getNumber(ConfigManager::POTION_EXAUSTION_GROUND_SPEED);
	}

	return ((1000 * groundSpeed) / stepSpeed) * lastStepCost;
}
