...
ItemType::ItemType() {
	...
	loaded = blockSolid = blockProjectile = blockPathFind = allowPickupable = false;
	moveable = true;
	alwaysOnTopOrder = 0;
	rotateTo = 0;
...
void Items::parseItemNode(xmlNodePtr itemNode, uint32_t id)
{
	...
	while(itemAttributesNode)
	{
		if(readXMLString(itemAttributesNode, "key", strValue))
		{
			...
			// <onepiece>
			else if(tmpStrValue == "savemanapercent")
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue))
					it.saveManaPercent = intValue;
			}
			else if(tmpStrValue == "extralootpercent")
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue))
					it.extraLootPercent = intValue;
			}
			else if(tmpStrValue == "vampirism_physical")
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue))
					it.vampirism_physical = intValue;
			}
			else if(tmpStrValue == "vampirism_magic")
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue))
					it.vampirism_magic = intValue;
			}
			else if(tmpStrValue == "extra_critical")
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue))
					it.extra_critical = intValue;
			}
			else if(tmpStrValue == "health_relative_damage")
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue))
					it.health_relative_damage = intValue;
			}
			else if(tmpStrValue == "charm_extra_dmg_chance")
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue))
					it.charm_extra_dmg_chance = intValue;
			}
			// </onepiece>
			else
