...
{
	m_muteConditionId = 0;
	m_muteCondition = NULL;
	m_accessToMute = 0;
	m_muteConditionMessage = "";
	
	configureChannelMuteConditions();
...
bool ChatChannel::talk(Player* player, SpeakClasses type, const std::string& text, uint32_t _time/* = 0*/)
{
	...
	if(m_muteConditionId > 0 && player && m_muteCondition && player->getAccess() >= m_accessToMute) {
		if(text[0] == '/') {
			if(text.substr(1, 4) == "mute") {
				std::string param = text.substr(5);
				trimString(param);	
				Player* paramPlayer = NULL;
				if(g_game.getPlayerByNameWildcard(param, paramPlayer) == RET_NOERROR) {
					if(Condition* condition = paramPlayer->getCondition(m_muteCondition->getType(), m_muteCondition->getId(), m_muteCondition->getSubId())) {
						player->sendCancel("That player is already muted.");
						return true;
					}
					
					if(Condition* condition = m_muteCondition->clone()) {
						paramPlayer->addCondition(condition);
						
						std::ostringstream ss;
						ss << paramPlayer->getName() << " has been muted for using " << m_name << " inappropriately.";
						
						for(it = m_users.begin(); it != m_users.end(); ++it) {
							it->second->sendToChannel(NULL, SPEAK_CHANNEL_RN, ss.str(), m_id, _time);		
						}						
					} else {
						player->sendCancel("Failed to create condition. Report to administrator.");
					}
				} else {
				...
				return true;
			} else if(text.substr(1, 6) == "unmute") {
				...
				if(g_game.getPlayerByNameWildcard(param, paramPlayer) == RET_NOERROR) {
					if(Condition* condition = paramPlayer->getCondition(m_muteCondition->getType(), m_muteCondition->getId(), m_muteCondition->getSubId())) {
						...
						std::ostringstream ss;
						ss << paramPlayer->getName() << " has been unmuted.";
...
void ChatChannel::configureChannelMuteConditions()
{
	//help channel, olhar no channels.xml
	if (m_id == 9) {
		//usar um conditionid que nao é usado no channels.xml conditionId="numero"
		int32_t conditionId = 20;
		int32_t muteForSeconds = 3600;
		if (m_muteCondition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_MUTED, muteForSeconds * 1000, 0, false, conditionId)) {
			m_muteConditionId = conditionId;
			//tutors podem mutar
			m_accessToMute = 1;
			m_muteConditionMessage = "You are muted from the Help channel for using it inappropriately.";
		}
	//game channel
	} else if(m_id == 5) {
		//usar um conditionid que nao é usado no channels.xml conditionId="numero"
		int32_t conditionId = 21;
		int32_t muteForSeconds = 3600;
		if (m_muteCondition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_MUTED, muteForSeconds * 1000, 0, false, conditionId)) {
			m_muteConditionId = conditionId;
			//gms podem mutar
			m_accessToMute = 1;
			m_muteConditionMessage = "You are muted from the Game Chat channel for using it inappropriately.";
		}
    //trade channel
	} else if(m_id == 6) {
		//usar um conditionid que nao é usado no channels.xml conditionId="numero"
		int32_t conditionId = 22;
		int32_t muteForSeconds = 3600;
		if (m_muteCondition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_MUTED, muteForSeconds * 1000, 0, false, conditionId)) {
			m_muteConditionId = conditionId;
			//gms podem mutar
			m_accessToMute = 1;
			m_muteConditionMessage = "You are muted from the Trade channel for using it inappropriately.";
		}
    //english channel
	} else if(m_id == 8) {
		//usar um conditionid que nao é usado no channels.xml conditionId="numero"
		int32_t conditionId = 23;
		int32_t muteForSeconds = 3600;
		if (m_muteCondition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_MUTED, muteForSeconds * 1000, 0, false, conditionId)) {
			m_muteConditionId = conditionId;
			//gms podem mutar
			m_accessToMute = 1;
			m_muteConditionMessage = "You are muted from the English Chat channel for using it inappropriately.";
		} 	
	}
}
...
bool Chat::talkToChannel(Player* player, SpeakClasses type, const std::string& text, uint16_t channelId)
{
	...
	if(!player->hasFlag(PlayerFlag_CannotBeMuted))
	{
		...
		if(channel->getMuteConditionId() >= 0 && player->hasCondition(CONDITION_MUTED, channel->getMuteConditionId()))
		{
			player->sendCancel(channel->getMuteConditionMessage().c_str());
			return true;
		}
	}
