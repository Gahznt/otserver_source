...
// <onepiece>
bool Condition::setParam(ConditionParam_t param, float value) {
	switch(param) {
...
Condition* Condition::createCondition(ConditionId_t _id, ConditionType_t _type, int32_t _ticks, int32_t param/* = 0*/, bool _buff/* = false*/, uint32_t _subId/* = 0*/)
{
	switch((int32_t)_type)
	{
        ...
        case CONDITION_FAST_ATTACK:
            return new ConditionFastAttack(_id, _type, _ticks, _buff, _subId);
        // </onepiece>
...
// <onepiece>
bool ConditionAttributes::setParam(ConditionParam_t param, float value)
{
	...
	switch(param)
	{
		...
		case CONDITIONPARAM_STAT_MAXHEALTHPERCENT:
			statsPercent[STAT_MAXHEALTH] = std::max((int32_t)0, (int32_t)value);
			return true;

		case CONDITIONPARAM_STAT_MAXMANAPERCENT:
			statsPercent[STAT_MAXMANA] = std::max((int32_t)0, (int32_t)value);
			return true;

		case CONDITIONPARAM_STAT_SOULPERCENT:
			statsPercent[STAT_SOUL] = std::max((int32_t)0, (int32_t)value);
			return true;

		case CONDITIONPARAM_STAT_MAGICLEVELPERCENT:
			statsPercent[STAT_MAGICLEVEL] = std::max((int32_t)0, (int32_t)value);
			return true;

		case CONDITIONPARAM_SKILL_MELEEPERCENT:
			skillsPercent[SKILL_CLUB] = skillsPercent[SKILL_AXE] = skillsPercent[SKILL_SWORD] = std::max((int32_t)0, (int32_t)value);
			return true;

		case CONDITIONPARAM_SKILL_FISTPERCENT:
			skillsPercent[SKILL_FIST] = std::max((int32_t)0, (int32_t)value);
			return true;

		case CONDITIONPARAM_SKILL_CLUBPERCENT:
			skillsPercent[SKILL_CLUB] = std::max((int32_t)0, (int32_t)value);
			return true;

		case CONDITIONPARAM_SKILL_SWORDPERCENT:
			skillsPercent[SKILL_SWORD] = std::max((int32_t)0, (int32_t)value);
			return true;

		case CONDITIONPARAM_SKILL_AXEPERCENT:
			skillsPercent[SKILL_AXE] = std::max((int32_t)0, (int32_t)value);
			return true;

		case CONDITIONPARAM_SKILL_DISTANCEPERCENT:
			skillsPercent[SKILL_DIST] = std::max((int32_t)0, (int32_t)value);
			return true;

		case CONDITIONPARAM_SKILL_SHIELDPERCENT:
			skillsPercent[SKILL_SHIELD] = std::max((int32_t)0, (int32_t)value);
			return true;

		case CONDITIONPARAM_SKILL_FISHINGPERCENT:
			skillsPercent[SKILL_FISH] = std::max((int32_t)0, (int32_t)value);
			return true;
...
ConditionFastAttack::ConditionFastAttack(ConditionId_t _id, ConditionType_t _type, int32_t _ticks, bool _buff, uint32_t _subId):
ConditionGeneric(_id, _type, _ticks, _buff, _subId)
{
	fastAttack = 0;
}

bool ConditionFastAttack::startCondition(Creature* creature) {

	if(Player* player = creature->getPlayer()) {
        player->setConditionFastAttack(fastAttack);	   
	}
	return Condition::startCondition(creature);
}

void ConditionFastAttack::addCondition(Creature* creature, const Condition* addCondition) {
    if(!updateCondition(addCondition))
		return;

	setTicks(addCondition->getTicks());
	const ConditionFastAttack& conditionFast = static_cast<const ConditionFastAttack&>(*addCondition);

    fastAttack  = conditionFast.fastAttack;
    if (Player * player = creature->getPlayer()) {
       player->setConditionFastAttack(fastAttack);
    }
}

void ConditionFastAttack::endCondition(Creature* creature, ConditionEnd_t reason)
{
	Player* player = creature->getPlayer();
	if(!player)
		return;

	player->setConditionFastAttack(0);
}

bool ConditionFastAttack::unserializeProp(ConditionAttr_t attr, PropStream& propStream)
{
	return ConditionGeneric::unserializeProp(attr, propStream);
}

bool ConditionFastAttack::serialize(PropWriteStream& propWriteStream)
{
	if(!ConditionGeneric::serialize(propWriteStream))
		return false;
...
bool ConditionFastAttack::executeCondition(Creature* creature, int32_t interval)
{     
    return ConditionGeneric::executeCondition(creature, interval);
}

bool ConditionFastAttack::setParam(ConditionParam_t param, float value)
{
	bool ret = ConditionGeneric::setParam(param, value);
	switch(param)
	{
...
Icons_t ConditionFastAttack::getIcons() const
{
	Icons_t icon = Condition::getIcons();
	if(icon != ICON_NONE)
		return icon;
...
bool ConditionRegeneration::executeCondition(Creature* creature, int32_t interval) {
    ...
    // <onepiece>	
    int32_t regen_multiplier = 100;
    if (Player * player = creature->getPlayer())
        regen_multiplier += player->getExtraFoodRegen();
     
	internalHealthTicks += interval * (regen_multiplier / 100);
	internalManaTicks   += interval * (regen_multiplier / 100);
	// </onepiece>	
	...
	if(creature->getZone() != ZONE_PROTECTION)
    {
        if(internalHealthTicks >= healthTicks)
		{
			...
			/*
            if(Player* player = creature->getPlayer())
			    creature->changeHealth(healthGain + player->getLevel() / 10);
            else // regen creatures (sera que existe esse caso ? RS 
...
// <onepiece>
bool ConditionRegeneration::setParam(ConditionParam_t param, float value)
{
	bool ret = ConditionGeneric::setParam(param, value);
	switch(param)
	{
...
// <onepiece>
bool ConditionSoul::setParam(ConditionParam_t param, float value)
{
	bool ret = ConditionGeneric::setParam(param, value);
	switch(param)
	{
...
// <onepiece>
bool ConditionDamage::setParam(ConditionParam_t param, float value)
{
...
// <onepiece>
bool ConditionSpeed::setParam(ConditionParam_t param, float value)
{
...
// <onepiece>
bool ConditionLight::setParam(ConditionParam_t param, float value)
{
	bool ret = Condition::setParam(param, value);
	switch(param)
	{
