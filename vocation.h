...
class Vocation
{
	public:
        ...
        // <onepiece>
		float getGainTicks(gain_t type) const { return gainTicks[type]; }
		void setGainTicks(gain_t type, float v) { gainTicks[type] = v; }
        // </onepiece>
	...
	private:
		...
		int16_t absorb[COMBAT_LAST + 1]
              , reflect[REFLECT_LAST + 1][COMBAT_LAST + 1]
              ;
		uint32_t gain[GAIN_LAST + 1]
               , gainAmount[GAIN_LAST + 1]
               , skillBase[SKILL_LAST + 1]
               ;
        // <onepiece>
        float gainTicks[GAIN_LAST + 1];
        // </onepiece>
               
		float skillMultipliers[SKILL__LAST + 1]
            , formulaMultipliers[MULTIPLIER_LAST + 1]
            ;
};
