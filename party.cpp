...
// <onepiece>
void Party::broadcastLootMessage(const std::string& text)
{
	PlayerVector::iterator it;
	if(!memberList.empty()) {
		for(it = memberList.begin(); it != memberList.end(); ++it) {
			(*it)->sendChannelMessage("[SV]", text, SPEAK_CHANNEL_W, CHANNEL_PARTY);
        }
	}

	leader->sendChannelMessage("[SV]", text, SPEAK_CHANNEL_W, CHANNEL_PARTY);
}
// </onepiece>
...
{
    std::vector<int32_t> vocationList; 
    for(PlayerVector::iterator it = memberList.begin(); it != memberList.end(); ++it)
	{
        if(Player* player = (*it)->getPlayer())
        {
            std::vector<int32_t>::iterator it;
            it = std::find (vocationList.begin(), vocationList.end(), player->getVocationId());
            if (!(it != vocationList.end()))
                vocationList.push_back(player->getVocationId());
        }
    } 
	double shareExperience = (experience / (double)(memberList.size() + 1));
	if((20*vocationList.size()) >= 100)
	    shareExperience += shareExperience;
    else
	    shareExperience += ((shareExperience/100)*(20*vocationList.size()));
