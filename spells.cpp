...
void Spell::postCastSpell(Player* player, uint32_t manaCost, uint32_t soulCost) const
{
	...
	{
        // <onepiece>
        manaCost = (int32_t)((float)manaCost * (100 - player->getSaveManaPercent()) / 100);
		player->changeMana(-(int32_t) manaCost);
		//std::cout << "MANASAVE: " << player->getSaveManaPercent() << std::endl;
		// </onepiece>
...
bool InstantSpell::Levitate(const InstantSpell* spell, Creature* creature, const std::string& param) {
	...
	bool try_up   = ! player->getNavigating() && currentPos.z != 8;
    bool try_down = ! player->getNavigating() && currentPos.z != 7;
    ...
    // jump up and jump down failed, lets try magic rope
    Tile * fromTile = g_game.getTile(currentPos);
    if (fromTile && fromTile->ground) {
        uint16_t tile_id = fromTile->ground->getID();
        if ((tile_id == 384 || tile_id == 418 || tile_id == 8278 || tile_id == 8592) || g_game.findItemOfType(fromTile, 18936, false, -1)) {
            Tile * toTile = g_game.getTile(currentPos.x, currentPos.y + 1, currentPos.z - 1);
            if (toTile && toTile->ground) {
                ret = g_game.internalMoveCreature(NULL, player, fromTile, toTile, FLAG_IGNOREBLOCKITEM | FLAG_IGNOREBLOCKCREATURE);
        		if (ret == RET_NOERROR) {
		            g_game.addMagicEffect(player->getPosition(), MAGIC_EFFECT_TELEPORT, player->isGhost());
		            return true;
    	        }
