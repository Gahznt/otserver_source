...
// <onepiece>
inline int32_t Item::getSaveManaPercent() const
{
	const int32_t* v = getIntegerAttribute("saveManaPercent");
	if(v)
		return *v;

	return items[id].saveManaPercent;
}

inline int32_t Item::getExtraLootPercent() const
{
	const int32_t* v = getIntegerAttribute("extraLootPercent");
	if(v)
		return *v;

	return items[id].extraLootPercent;
}

inline int32_t Item::getVampirismPhysical() const
{
	const int32_t* v = getIntegerAttribute("vampirism_physical");
	if(v)
		return *v;

	return items[id].vampirism_physical;
}

inline int32_t Item::getVampirismMagic() const
{
	const int32_t* v = getIntegerAttribute("vampirism_magic");
	if(v)
		return *v;

	return items[id].vampirism_magic;
}

inline int32_t Item::getExtraCritical() const
{
	const int32_t* v = getIntegerAttribute("extra_critical");
	if(v)
		return *v;

	return items[id].extra_critical;
}

inline int32_t Item::getHealthRelativeDamage() const
{
	const int32_t* v = getIntegerAttribute("health_relative_damage");
	if(v)
		return *v;

	return items[id].health_relative_damage;
}

inline int32_t Item::getCharmExtraDmgChance() const
{
	const int32_t* v = getIntegerAttribute("charm_extra_dmg_chance");
	if(v)
		return *v;

	return items[id].charm_extra_dmg_chance;
}
// </onepiece>
