...
class ItemType
{
	...
	public:
		...
		bool loaded, stopTime, showCount, clientCharges, stackable, showDuration, showCharges, showAttributes,
        ...
        // <onepiece>
        uint16_t saveManaPercent; // % de mana que vai salvar ao soltar spell 
        uint16_t extraLootPercent; // acho que nao vai dar pra usar +
        uint16_t vampirism_physical; // % de vida que vai roubar do oponente ao dar dano fisico
        uint16_t vampirism_magic;    // % de vida que vai roubar do oponente ao dar dano magico
        uint16_t extra_critical;     // % de critical extra que o item da
        uint16_t health_relative_damage; // Dano extra baseado na vida do target
        uint16_t charm_extra_dmg_chance; // chance de dar um dano extra de acordo com o seu level
        // </onepiece>
