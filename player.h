...
class Player : public Creature, public Cylinder
{
	public:
        ...
        // <onepiece>
        uint32_t getBaseVocationId() const { return vocation_id % 10; }
        
        bool getNavigating() const { return isNavigating; }
		void setNavigating(bool __enabled) { isNavigating = __enabled; }

		uint32_t getExtraCriticalChance() const; // agora em player.cpp
		void setExtraCriticalChance(uint8_t __extra_chance) { extraCriticalChance = __extra_chance; }

        uint8_t getExtraFoodRegen() const { return extraFoodRegen; }
		void setExtraFoodRegen(uint8_t __food_regen) { extraFoodRegen = __food_regen; }

        uint8_t getExtraFoodDmg() const { return extraFoodDmg; }
		void setExtraFoodDmg(uint8_t __food_dmg) { extraFoodDmg = __food_dmg; }
		
		uint8_t getExtraFoodDef() const { return extraFoodDef; }
		void setExtraFoodDef(uint8_t __food_def) { extraFoodDef = __food_def; }

		uint32_t getConditionFastAttack() const { return conditionFastAttack; }
		void setConditionFastAttack(uint32_t __conditionFastAttack) { conditionFastAttack = __conditionFastAttack; }
        ...
        // </onepiece>
                // Fun��es que percorrem os EQs e calcula os bonus de determinada propriedade
        int32_t getSaveManaPercent() const;
        int32_t getExtraLootPercent() const;
        int32_t getVampirismPhysical() const;
        int32_t getVampirismMagic() const;
        int32_t getExtraCritical() const;
        int32_t getHealthRelativeDamage() const;
        int32_t getCharmExtraDmgChance() const;
        
        float getOutfitExtraAttack() const;
        float getOutfitExtraDefense() const;
        // </onepiece>
		...
		void setNextMoveAction(int64_t time) {if(time > nextMoveAction) {nextMoveAction = time;}}
		bool canDoMoveAction() const {return nextMoveAction <= OTSYS_TIME();}
		uint32_t getNextMoveActionTime() const;
	...
	private:
        ...
        // <onepiece>
        bool     isNavigating;
        uint8_t  extraCriticalChance;
        uint8_t  extraFoodRegen;
        uint8_t  extraFoodDmg;
        uint8_t  extraFoodDef;
        uint32_t conditionFastAttack;
        // </onepiece>
