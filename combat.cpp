...
ReturnValue Combat::canDoCombat(const Creature* attacker, const Creature* target)
{
	...
	if(const Player* targetPlayer = target->getPlayer())
	{
		...
		{
			if(attackerPlayer->getSecureMode() == SECUREMODE_ON) {
				return RET_YOUMAYNOTATTACKTHISPLAYER;
			}
...
bool Combat::CombatHealthFunc(Creature* caster, Creature* target, const CombatParams& params, void* data)
{
    ...
    // Luffy bloqueia ataque fisico em X%
    if(params.combatType == COMBAT_PHYSICALDAMAGE) {
      if (target && target->getPlayer() && target->getPlayer()->getBaseVocationId() == 2) {
        //std::cout << "[DEBUG] __1533 b4 (" << change << "," << g_config.getDouble(ConfigManager::WOP_LUFFY_BLOCK_PHYSICAL) << ")" << std::endl;
        change = (int32_t) ((float)change * (1.0 - (g_config.getDouble(ConfigManager::WOP_LUFFY_BLOCK_PHYSICAL) / 100)));
        //std::cout << "[DEBUG] __1533 af7 (" << change << "," << g_config.getDouble(ConfigManager::WOP_LUFFY_BLOCK_PHYSICAL) << ")" << std::endl;
      }
    }
    
    // Red skull recebe mais dano de player que o normal
    if (caster && caster->getPlayer() && target && target->getPlayer() && target->getPlayer()->getSkull() >= SKULL_RED)
      change = (int32_t) ((float)change * g_config.getDouble(ConfigManager::OPC_RED_SKULL_PLAYER_DAMAGE));
    
    // Dependendo do outfit da pessoa, ele aumentar o percentual de defesa
    if(target && target->getPlayer())
      change = (int32_t) ((float)change / target->getPlayer()->getOutfitExtraDefense());
    
    // Dependendo da food que ele come ele tem bonus de aumentar a defesa em %
    if (target && target->getPlayer())
       change = (int32_t) ((float) change * ((100.0 - target->getPlayer()->getExtraFoodDef()) / 100.0)) ;
    
    // Dependendo da food que o attacker come, ele tem bonus de aumentar o dano em % 
    if (caster && caster->getPlayer())
       change = (int32_t) ((float) change * ((100.0 + caster->getPlayer()->getExtraFoodDmg()) / 100.0)) ;
    
    // Spiny Tortoise as vezes usa uma magia que troca para um outfit identico, e este faz tomar metade do dano
    if(target && target->getCurrentOutfit().lookType == 230)
      change = (int32_t) (change * 0.5);

    // Precisa ser por ultimo os vampirismos
    if (caster && caster->getPlayer()) {
        // Relative Health Damage  
        if (target)     
            change -= (int32_t) caster->getPlayer()->getHealthRelativeDamage() * target->getHealth() * 0.01;
        
        // Vampirismo Fisico
        int32_t steal_health = -(int32_t) change * ( params.combatType <= COMBAT_PHYSICALDAMAGE
                                                   ? caster->getPlayer()->getVampirismPhysical()
                                                   : caster->getPlayer()->getVampirismMagic()
                                                   ) / 100;
        if (steal_health > 0) {
            caster->changeHealth(steal_health);
        }
    }    
