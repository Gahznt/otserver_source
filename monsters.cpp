...
void MonsterType::dropLoot(Container* corpse)
{
	...
	if(owner->getParty() && message > LOOTMSG_PLAYER){
        ...
        // <onepiece>
		// Mandar mensagem de loot no canal da Party 
		owner->getParty()->broadcastLootMessage(ss.str());
		// </onepiece>
...
{
	loaded = false;
	xmlDocPtr doc = xmlParseFile(getFilePath(FILE_TYPE_OTHER, "monster/_monsters.xml").c_str());
	if(!doc)
	{
