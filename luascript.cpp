...
void LuaScriptInterface::registerFunctions()
{
    ...
    // <onepiece>
      ...
      // setCreatureVisible(cid, visible)
	  lua_register(m_luaState, "setCreatureVisible", LuaScriptInterface::luaInternalCreatureChangeVisible);
	  
	  //setMonsterName(cid, name, description)          (O nome antes era setCreatureName, mas eu nao gostei)
      lua_register(m_luaState, "setMonsterName", LuaScriptInterface::luaSetMonsterName);

      // doCreateMonsterForced(name, pos)
	  lua_register(m_luaState, "doCreateMonsterForced", LuaScriptInterface::luaDoCreateMonsterForced);


	  //doPlayerAddSkillLevel(cid, skill, level) 
      lua_register(m_luaState, "doPlayerAddSkillLevel", LuaScriptInterface::luaDoPlayerAddSkillLevel);
      
      //doPlayerSetTarget(cid, target)
      lua_register(m_luaState, "doPlayerSetTarget", LuaScriptInterface::luaDoPlayerSetTarget);
      
      //getPlayerIsNavigating(cid)
      lua_register(m_luaState, "getPlayerIsNavigating", LuaScriptInterface::luaGetPlayerIsNavigating);
      
      //setPlayerIsNavigating(cid, enable)
      lua_register(m_luaState, "setPlayerIsNavigating", LuaScriptInterface::luaSetPlayerIsNavigating);
      ...
      //setPlayerCriticalExtraChance(cid, extra_chance)
      lua_register(m_luaState, "setPlayerCriticalExtraChance", LuaScriptInterface::luaSetPlayerCriticalExtraChance);
      
      //setPlayerExtraFoodRegen(cid, extra_regen_percent)
      lua_register(m_luaState, "setPlayerExtraFoodRegen", LuaScriptInterface::luaSetPlayerExtraFoodRegen);
      
      //setPlayerExtraFoodDmg(cid, extra_dmg_percent)
      lua_register(m_luaState, "setPlayerExtraFoodDmg", LuaScriptInterface::luaSetPlayerExtraFoodDmg);
      
      //setPlayerExtraFoodDef(cid, extra_def_percent)
      lua_register(m_luaState, "setPlayerExtraFoodDef", LuaScriptInterface::luaSetPlayerExtraFoodDef);
      
      //getContainerItemEx(cid, container_index, slot_index)
      lua_register(m_luaState, "getContainerItemEx", LuaScriptInterface::luaGetContainerItemEx);
      
      //getCreatureDefaultOutfit(cid)
      lua_register(m_luaState, "getCreatureDefaultOutfit", LuaScriptInterface::luaGetCreatureDefaultOutfit);
      
      //getPlayerCharmExtraDamageChance(cid)
      lua_register(m_luaState, "getPlayerCharmExtraDamageChance", LuaScriptInterface::luaGetPlayerCharmExtraDamageChance);
...
// <onepiece>
int32_t LuaScriptInterface::luaInternalCreatureChangeVisible(lua_State* L)
{
	// setCreatureVisible(uid, visible)
	uint32_t visible = popNumber(L);

	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		g_game.internalCreatureChangeVisible(creature, (Visible_t) visible);
		lua_pushboolean(L, true);
	}
	else
	{
...
int32_t LuaScriptInterface::luaSetMonsterName(lua_State* L)
{
...
int32_t LuaScriptInterface::luaDoCreateMonsterForced(lua_State* L) {
	//doCreateMonsterForced(name, pos)
	PositionEx pos;
	popPosition(L, pos);
...
int32_t LuaScriptInterface::luaDoPlayerAddSkillLevel(lua_State* L)
{
	//doPlayerAddSkillLevel(cid, skill, count) 
	int32_t count = (int32_t) popNumber(L);
    skills_t skill_id = (skills_t) popNumber(L);	
	...
	if(Player* player = env->getPlayerByUID(popNumber(L)))
	{
		player->addSkillAdvance(skill_id, (uint32_t)(player->getVocation()->getReqSkillTries(skill_id, player->getSkill(skill_id, SKILL_LEVEL) + count) - player->getSkill(skill_id, SKILL_TRIES)), false);
		lua_pushboolean(L, true);
	}
	else
	{
...
int32_t LuaScriptInterface::luaDoPlayerSetTarget(lua_State* L)
{
	...
	if(Player * player = env->getPlayerByUID(popNumber(L)))
    {
	    if (! target && player->getAttackedCreature())
...
int32_t LuaScriptInterface::luaGetPlayerIsNavigating(lua_State* L)
{
    //getPlayerIsNavigating(cid)
    ScriptEnviroment* env = getEnv();
    if(Player* player = env->getPlayerByUID(popNumber(L)))
    {
        lua_pushboolean(L, player->getNavigating());
    }
    else
    {
        errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));        
...
int32_t LuaScriptInterface::luaSetPlayerIsNavigating(lua_State* L)
{
    //setPlayerIsNavigating(cid, enable)
    ScriptEnviroment* env = getEnv();
    int32_t enable = (int32_t) popNumber(L);
    
    if(Player* player = env->getPlayerByUID(popNumber(L)))
    {
        player-> setNavigating(enable);
        lua_pushboolean(L, true);
    }
    else
    {
...
int32_t LuaScriptInterface::luaSetPlayerCriticalExtraChance(lua_State* L)
{
    //setPlayerCriticalExtraChance(cid, extra_chance)
    ScriptEnviroment* env = getEnv();
    uint8_t extra_chance = (uint8_t) popNumber(L);
    
    if(Player* player = env->getPlayerByUID(popNumber(L)))
    {
        player->setExtraCriticalChance(extra_chance);
        lua_pushboolean(L, true);
    }
    else
    {
...
int32_t LuaScriptInterface::luaSetPlayerExtraFoodRegen(lua_State* L) {
    //setPlayerExtraFoodRegen(cid, extra_regen_percent)

    ScriptEnviroment* env = getEnv();
    uint8_t extra_regen_percent = (uint8_t) popNumber(L);
    
    if(Player* player = env->getPlayerByUID(popNumber(L)))
    {
        player->setExtraFoodRegen(extra_regen_percent);
        lua_pushboolean(L, true);
    }
    else
    {
...
int32_t LuaScriptInterface::luaSetPlayerExtraFoodDmg(lua_State* L) {
    //setPlayerExtraFoodDmg(cid, extra_dmg_percent)

    ScriptEnviroment* env = getEnv();
    uint8_t extra_dmg_percent = (uint8_t) popNumber(L);
    
    if(Player* player = env->getPlayerByUID(popNumber(L)))
    {
        player->setExtraFoodDmg(extra_dmg_percent);
        lua_pushboolean(L, true);
    }
    else
    {
...
int32_t LuaScriptInterface::luaSetPlayerExtraFoodDef(lua_State* L) {
    //setPlayerExtraFoodDef(cid, extra_def_percent)

    ScriptEnviroment* env = getEnv();
    uint8_t extra_def_percent = (uint8_t) popNumber(L);
    
    if(Player* player = env->getPlayerByUID(popNumber(L)))
    {
        player->setExtraFoodDef(extra_def_percent);
        lua_pushboolean(L, true);
    }
    else
    {
...
int32_t LuaScriptInterface::luaGetContainerItemEx(lua_State* L)
{
	//getContainerItemEx(cid, container_index, slot_index)
...
int32_t LuaScriptInterface::luaGetCreatureDefaultOutfit(lua_State* L)
{
	//getCreatureDefaultOutfit(cid)
	ScriptEnviroment* env = getEnv();
...
int32_t LuaScriptInterface::luaGetPlayerCharmExtraDamageChance(lua_State* L) {
        
	//getPlayerCharmExtraDamageChance(cid)
	ScriptEnviroment* env = getEnv();
	if(const Player* player = env->getPlayerByUID(popNumber(L))) {
		lua_pushnumber(L, player->getCharmExtraDmgChance());
	} else {
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
		lua_pushboolean(L, false);
	}
...
{    
     //doPlayerAddAttackSpeed One Piece
