...
void ProtocolGame::parsePacket(NetworkMessage &msg)
{
	...
	if(player->isAccountManager())
	{
		switch(recvbyte)
		{
			...
			case 0x96:
            //case 0x96: // no cliente normal 0x96 era original, agora fiz -1
				parseSay(msg);
				break;
	...
	else
	{
		switch(recvbyte)
		{
          ...
          // </onepiece>
            ...
            //case 0x96: // say something tibia client
				parseSay(msg);
				break;
			...
			//case 0xA1: // attack (original)
			case 0xA1: // attack
				parseAttack(msg);
				break;

			//case 0xA2: //follow (original)
			case 0xA2: //follow
				parseFollow(msg);
				break;
...
void ProtocolGame::AddPlayerStats(NetworkMessage_ptr msg)
{
	...
	// <onepiece>
    msg->AddU16(player->getPlayerInfo(PLAYERINFO_SOUL));
    // </onepiece>
	...
	std::string text, uint16_t channelId, uint32_t time/*= 0*/, Position* pos/* = NULL*/)
{
	...
	if(creature)
	{
		...
		if(speaker)
		{
			...
			if(channelId == 9 && player->getGroupId() < 3) {
				uint32_t speakerGroupId = speaker->getGroupId();
				if (speakerGroupId == 2 || speakerGroupId == 3) {
					hideLevel = true;
					if (speakerGroupId == 2) {
						creatureName = "Tutor";
					} else {
						creatureName = "Senior Tutor";
					}
				}
			}
		...
		switch(type)
		{
			...
			case SPEAK_RVR_ANSWER:
				...
				break;
			default:
				msg->AddString(!creature->getHideName() ? creatureName : "");
				break;
		}

		if(!hideLevel && speaker && type != SPEAK_RVR_ANSWER && !speaker->isAccountManager()
