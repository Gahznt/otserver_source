...
bool Weapon::useFist(Player* player, Creature* target)
{
	...
	if(random_range(1, 100) <= g_config.getNumber(ConfigManager::CRITICAL_HIT_CHANCE) + player->getExtraCriticalChance())
	{
...
bool Weapon::internalUseWeapon(Player* player, Item* item, Creature* target, int32_t damageModifier) const
{
	...
	else
	{
		...
		// <onepiece> Smoker e Sanji na magia Diable Jambe
		if (player->getCurrentOutfit().lookType == 92 || player->getCurrentOutfit().lookType == 94){
            CombatParams smoker;
        	smoker.blockedByArmor  = false;
        	smoker.blockedByShield = false;
            smoker.combatType      = COMBAT_FIREDAMAGE;
            smoker.effects.hit     = (MagicEffect_t) 226;
            smoker.effects.color   = (TextColor_t) TEXTCOLOR_WHITE;
            Combat::doCombatHealth(player, target, damage, damage, smoker);
        } else if (player->getCurrentOutfit().lookType == 485 || player->getCurrentOutfit().lookType == 486) {
            CombatParams sanji;
        	sanji.blockedByArmor  = false;
        	sanji.blockedByShield = false;
            sanji.combatType      = COMBAT_FIREDAMAGE;
            sanji.effects.hit     = (MagicEffect_t) 15;
            sanji.effects.color   = (TextColor_t) TEXTCOLOR_ORANGE;
            Combat::doCombatHealth(player, target, damage, damage, sanji);
        } else {
