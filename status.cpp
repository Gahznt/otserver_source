...
std::string Status::getStatusString(bool sendPlayers) const
{
    ...
    // <onepiece> -- retirado as infos que nao serao mais usadas
    ...
    */
    ...
    sprintf(buffer, "%d", Connection::connectionCount); //sprintf(buffer, "%d", g_game.getPlayersOnline());
...
void Status::getInfo(uint32_t requestedInfo, OutputMessage_ptr output, NetworkMessage& msg) const
{
	...
	if(requestedInfo & REQUEST_PLAYERS_INFO)
	{
		...
		// <onepiece>
          ...
          output->AddU32(Connection::connectionCount);
        // </onepiece>
